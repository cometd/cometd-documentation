...
NOTE: The rest of this document speaks as though HTTP will be used for message transport. 
...
subscribe (see also <<_bayeux_subscribe,the bayeux subscribe section>>) to meta channels.
...
A server MUST handle any other message sent in the same request as a connect message
after the handling of the connect message is complete.
...
+successful+::
  boolean indicating the success or failure of the publish
...
cookies to detect multiple intances of Bayeux clients running within the same HTTP client.
