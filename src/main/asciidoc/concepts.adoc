...
TODO: review all listeners, or perhaps move them down to the processing section
...
* +ServerSession+ allows you add listeners that can interact with the server
  session's message queue for example to detect whether the queue is too big via
  +ServerSession.addListener(ServerSession.MaxQueueListener)+, or to add/remove/coalesce
  messages just before they are sent via +ServerSession.addListener(ServerSession.DeQueueListener)+.
...
// TODO: use a sidebar here ?
For example: 

Remote client +client1+ wants to send a message to another remote client +client2+.
Both clients are already connected and therefore have already performed the
