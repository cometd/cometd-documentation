...
. Unpack the dojo-release-[version].tar.gz to a directory, for example +/tmp+,
  so that you have the +/tmp/dojo-release-[version]+ directory that we will call
  +$DOJO+.
. Delete the +$DOJO/dojox/cometd.js+ and +$DOJO/dojox/cometd.js.uncompressed.js+ files
  ...
  we will put in place in a moment).
...
[source,javascript]
----
include::{raw-github}/cometd-archetypes/dojo-jetty9/src/main/resources/archetype-resources/src/main/webapp/application.js[]
----
====
