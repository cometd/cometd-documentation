...
you need a reference to the +cometd+ object that has the logging methods,
and you need only the outgoing extension method:
...
Notice that meta messages are also passed to the extension methods; you
normally have to filter the messages that the extension method receives
...
The +outgoing()+ and +incoming()+ methods can avoid  returning something, or
...
If the extension method returns +null+, the processing should stop: other
...
registration order: outgoing extensions methods are called in registration
order and, by default, incoming registration methods are called in reverse
registration order.
...
methods called are: +extA.outgoing()+ and then +extB.outgoing()+, while for
incoming messages the methods called are +extB.incoming()+ and then +extA.incoming()+.
...
every time an extension throws an exception (for example, calling a method on an
undefined object):
